...
const int clientSearchTimeout = 3;		//number of seconds to search for client if server
...
char*  Network::intToIpAddr(long ipAddress){
	return intToIpAddr(ipAddress, true);
}


char*  Network::intToIpAddr(long ipAddress, bool networkByteOrder){
...
void Network::waitForClientConnection(){
	...
	while(!connectionOpen && millisecondsWaited<=clientSearchTimeout*1000){
		broadcastToClients("255.255.255.255");
		broadcastToClients(intToIpAddr(theirIp)); //hack to get Network  to work over internet as well as LAN
...
void Network::broadcastToClients(const char* host){
...
void Network::readPacketToBuffer(){
